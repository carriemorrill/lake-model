      ...
      real d2Hp_i     ! interpolated delta 2H of precipitation for this dt [per mil]
      real d2Hr_i     ! interpolated delta 2H of runoff for this dt [per mil]
      real d18Op_i    ! interpolated delta 18O of precipitation for this dt [per mil]
 ...
 140  if (wb_flag) then
        if (hour_flag) then
          if(d2H_flag.or.d18O_flag) then
             read(15,*,end=998) year(1), mon(1), day(1), hour(1),  &
                     ta_in(1), hum_in(1), ua_in(1), sw_in(1),      &
                     rlwd_in(1), ps_in(1), prec_in(1), runin_in(1),&
                     d18Op_in(1), d18Or_in(1), d2Hp_in(1), d2Hr_in(1)
          else
             read(15,*,end=998) year(1), mon(1), day(1), hour(1),  &
                     ta_in(1), hum_in(1), ua_in(1), sw_in(1),      &
                     rlwd_in(1), ps_in(1), prec_in(1), runin_in(1)
          ...
          if(d2H_flag.or.d18O_flag) then
             read(15,*,end=998) year(1), mon(1), day(1),           &
                     ta_in(1), hum_in(1), ua_in(1), sw_in(1),      &
                     rlwd_in(1), ps_in(1), prec_in(1), runin_in(1),&
                     d18Op_in(1), d18Or_in(1), d2Hp_in(1), d2Hr_in(1)
          else
      ...
      else     ! no water balance calculations
        if (d2H_flag.or.d18O_flag) then
           print *, "To simulate isotopes, water balance flag must be set to true in lake.inc"
           goto 998
        endif
        if (hour_flag) then
 ...
 150  if (wb_flag) then
        if (hour_flag) then
          if(d2H_flag.or.d18O_flag) then
             read(15,*,end=998) year(2), mon(2), day(2), hour(2),  &
                     ta_in(2), hum_in(2), ua_in(2), sw_in(2),      &
                     rlwd_in(2), ps_in(2), prec_in(2), runin_in(2),&
                     d18Op_in(2), d18Or_in(2), d2Hp_in(2), d2Hr_in(2)
          else
             read(15,*,end=998) year(2), mon(2), day(2), hour(2),  &
                     ta_in(2), hum_in(2), ua_in(2), sw_in(2),      &
                     rlwd_in(2), ps_in(2), prec_in(2), runin_in(2)
          ...
          if(d2H_flag.or.d18O_flag) then
             read(15,*,end=998) year(2), mon(2), day(2),           &
                     ta_in(2), hum_in(2), ua_in(2), sw_in(2),      &
                     rlwd_in(2), ps_in(2), prec_in(2), runin_in(2),&
                     d18Op_in(2), d18Or_in(2), d2Hp_in(2), d2Hr_in(2)
          else
      ...
      do j = 1,nsteps
         ...
         call tendency (j, nsteps, ta_in, ta_i, qa_in, qa_i, ua_in,     &
                        ...
                        d2Hp_in, d2Hp_i, d18Op_in, d18Op_i,             &
                        d2Hr_in, d2Hr_i, d18Or_in, d18Or_i) 
